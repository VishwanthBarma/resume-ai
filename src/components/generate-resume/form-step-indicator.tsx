    ...
    <h1 className='text-sm'>Step {currentStep} of {steps.length}</h1>
        {
            steps.map((step, index) => (
                <div className='relative group'>
                    ...
                    {
                        currentStep != index+1 && index+1 <= latestStep && <p className='absolute -top-14 -left-12 bg-neutral-900 border-neutral-800 text-slate-100 w-28 h-12 text-center rounded-lg text-sm hidden group-hover:flex items-center justify-center border-2 '>{step}</p>
                    }
